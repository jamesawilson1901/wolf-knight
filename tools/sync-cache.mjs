...
if (!swDrift && !badgeDrift) {
  console.log(`ALL CLEAN — ${modules.length} modules and ${vo.length} spoken lines precached, `
    + `badge and CACHE_NAME both ${version}`);
  process.exit(0);
}
if (!WRITE) {
...
console.log(`written — ${modules.length} modules and ${vo.length} spoken lines precached, `
  + `badge and CACHE_NAME both ${version}`);
